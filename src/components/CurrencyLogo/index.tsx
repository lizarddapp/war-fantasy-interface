...
export const getTokenFallbackLogoURL = (currency: Currency) => `https://d1xrz6ki9z98vb.cloudfront.net/venomswap/tokens/${currency.symbol}.png`
...
export default function CurrencyLogo({ currency, size = '24px', style }: { currency?: Currency; size?: string; style?: React.CSSProperties }) {
  ...
  const srcs: string[] = useMemo(() => {
    ...
    if (currency instanceof Token) {
      const logoUrlLocation = [56, 97, 1666600000, 1666700000].includes(currency.chainId) ? getTokenFallbackLogoURL(currency) : getTokenLogoURL(currency.address)
  ...
  if (currency === ETHER) {
    ...
    } else if (currency === BINANCE_COIN || (currency === wrappedCurrency && blockchain === Blockchain.BINANCE_SMART_CHAIN)) {
