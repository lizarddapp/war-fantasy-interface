...
import { BigintIsh, Currency, CurrencyAmount, ETHER, JSBI, Pair, Route, Token, TokenAmount, Trade, TradeType, WETH } from '@venomswap/sdk'
...
export function useUserHasLiquidityInAllTokens(): boolean | undefined {
  ...
  const v1ExchangeLiquidityTokens = useMemo(() => (chainId ? Object.keys(exchanges).map(address => new Token(chainId, address, 18, 'UNI-V1', 'Uniswap V1')) : []), [
    chainId,
    exchanges
  ])
 ...
 */
export function useV1Trade(isExactIn?: boolean, inputCurrency?: Currency, outputCurrency?: Currency, exactAmount?: CurrencyAmount): Trade | undefined {
