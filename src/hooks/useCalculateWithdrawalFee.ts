...
function calculateFee(devFeeStage: BigNumber, divisor: number, multiply = '100'): Fraction | undefined {
  return devFeeStage && !devFeeStage.isZero() ? new Fraction(JSBI.BigInt(devFeeStage), JSBI.BigInt(divisor)).multiply(multiply) : undefined
}
...
export default function useCalculateWithdrawalFee(pid: number, account: string | null | undefined): Record<string, any> {
  ...
  if (lastWithdrawBlock.eq(BigNumber.from(0)) && firstDepositBlock.eq(BigNumber.from(0)) && blockDelta.eq(BigNumber.from(0)) && lastDepositBlock.eq(BigNumber.from(0))) {
  ...
  if (currentBlockBigNum && blockDeltaStartStages.length > 0 && blockDeltaEndStages.length > 0 && devFeeStages.length > 0) {
