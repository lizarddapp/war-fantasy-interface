...
export function useTokenBalance(account?: string, token?: Token, method = 'balanceOf', tokenInterface = ERC20_INTERFACE): TokenAmount | undefined {
...
export function useCurrencyBalances(account?: string, currencies?: (Currency | undefined)[]): (CurrencyAmount | undefined)[] {
  ...
  const containsETH: boolean = useMemo(() => currencies?.some(currency => currency && DEFAULT_CURRENCIES.includes(currency)) ?? false, [currencies])
...
export function useAggregateGovTokenBalance(): TokenAmount | undefined {
  ...
  return new TokenAmount(govToken, JSBI.add(JSBI.add(govTokenBalance?.raw ?? JSBI.BigInt(0), govTokenUnclaimed?.raw ?? JSBI.BigInt(0)), govTokenUnHarvested?.raw ?? JSBI.BigInt(0)))
}
